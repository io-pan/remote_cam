...
public class RNCameraView extends CameraView implements LifecycleEventListener, BarCodeScannerAsyncTaskDelegate, FaceDetectorAsyncTaskDelegate,
    MotionDetectorAsyncTaskDelegate, BarcodeDetectorAsyncTaskDelegate, TextRecognizerAsyncTaskDelegate, PictureSavedDelegate {
  ...
  public volatile boolean motionDetectorTaskLock = false;
  ...
  private boolean mShouldDetectMotion = false;
  private boolean mShouldGoogleDetectBarcodes = false;
  private boolean mShouldScanBarCodes = false;
  private boolean mShouldRecognizeText = false;
  private int mFaceDetectorMode = RNFaceDetector.FAST_MODE;
  private int mMotionDetectorMode = 0; // 0: running, returns only boolean; 1: preview, returns motion bitmap... -1 does not trigger.
  private int mMotionDetectorThreshold = 128;
  private int mMotionDetectorSampleSize = 20;
  private String mMotionDetectorArea = "";
  private int mMotionDetectorMinimumPixels = 1;
  ...
  public RNCameraView(ThemedReactContext themedReactContext) {
    ...
    addCallback(new Callback() {
      ...
      boolean willCallMotionTask = (mMotionDetectorMode>=0) && mShouldDetectMotion && !motionDetectorTaskLock && cameraView instanceof MotionDetectorAsyncTaskDelegate;
        boolean willCallGoogleBarcodeTask = mShouldGoogleDetectBarcodes && !googleBarcodeDetectorTaskLock && cameraView instanceof BarcodeDetectorAsyncTaskDelegate;
        boolean willCallTextTask = mShouldRecognizeText && !textRecognizerTaskLock && cameraView instanceof TextRecognizerAsyncTaskDelegate;
        if (!willCallMotionTask && !willCallBarCodeTask && !willCallFaceTask && !willCallGoogleBarcodeTask && !willCallTextTask) {
          return;
        }
        ...
        if (willCallMotionTask) {
          motionDetectorTaskLock = true;
          float density = getResources().getDisplayMetrics().density;
          MotionDetectorAsyncTaskDelegate delegate = (MotionDetectorAsyncTaskDelegate) cameraView;
          new MotionDetectorAsyncTask(delegate, mMotionDetector, data, width, height, getWidth(), getHeight(), density, correctRotation).execute();
        }
  ...
  public void setShouldScanBarCodes(boolean shouldScanBarCodes) {
    ...
    setScanning(mShouldDetectFaces || mShouldDetectMotion|| mShouldGoogleDetectBarcodes || mShouldScanBarCodes || mShouldRecognizeText);
  }
  ...
  private void setupMotionDetector() {
    mMotionDetector = new MotionDetector(mThemedReactContext);
    mMotionDetector.setMode(mMotionDetectorMode);
    mMotionDetector.setThreshold(mMotionDetectorThreshold);
    mMotionDetector.setMinimumPixels(mMotionDetectorMinimumPixels);
    mMotionDetector.setArea(mMotionDetectorArea);
    mMotionDetector.setSampleSize(mMotionDetectorSampleSize);
    mMotionDetector.setTracking(true);
  }

  public void setMotionDetectionMinimumPixels(int pixelsNumber) {
    mMotionDetectorMinimumPixels = pixelsNumber;
    if (mMotionDetector != null) {
      mMotionDetector.setMinimumPixels(pixelsNumber);
    }
  }

  public void setMotionDetectionMode(int mode) {
    mMotionDetectorMode = mode;
    if (mMotionDetector != null) {
      mMotionDetector.setMode(mode);
    }
  }

  public void setMotionDetectionThreshold(int threshold) {
    mMotionDetectorThreshold = threshold;
    if (mMotionDetector != null) {
      mMotionDetector.setThreshold(threshold);
    }
  }

  public void setMotionDetectionSampleSize(int size) {
    mMotionDetectorSampleSize = size;
    if (mMotionDetector != null) {
      mMotionDetector.setSampleSize(size);
    }
  }

  public void setMotionDetectionArea(String area) {
    mMotionDetectorArea = area;
    if (mMotionDetector != null) {
      mMotionDetector.setArea(area);
    }
  }


  public void setShouldDetectMotion(boolean shouldDetectMotion) {
    if (shouldDetectMotion && mMotionDetector == null) {
      setupMotionDetector();
    }
    this.mShouldDetectMotion = shouldDetectMotion;
    setScanning(mShouldDetectMotion || mShouldDetectFaces || mShouldGoogleDetectBarcodes || mShouldScanBarCodes || mShouldRecognizeText);
  }
  ...
  /**
   * Initial setup of the motion detector
   */
  public void onMotionDetected(WritableMap motionReported, int sourceWidth, int sourceHeight, int sourceRotation) {
    if (!mShouldDetectMotion) {
      return;
    }

    WritableMap motionDetected = motionReported == null ? Arguments.createMap() : motionReported;

    ImageDimensions dimensions = new ImageDimensions(sourceWidth, sourceHeight, sourceRotation, getFacing());
    RNCameraViewHelper.emitMotionDetectedEvent(this, motionDetected, dimensions);
  }

  public void onMotionDetectionError(MotionDetector motionDetector) {
    if (!mShouldDetectMotion) {
      return;
    }

    RNCameraViewHelper.emitMotionDetectionErrorEvent(this, motionDetector);
  }

  @Override
  public void onMotionDetectingTaskCompleted() {
    motionDetectorTaskLock = false;
  }
